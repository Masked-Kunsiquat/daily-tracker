      ...
      <TouchableOpacity style={styles.container} accessibilityRole="button">
        ...
        <View style={styles.stats}>
          <Text style={styles.statText}>
            ✅ {entry.accomplishments.length} • 📚 {entry.things_learned.length} • 🙏{' '}
            {entry.things_grateful.length}
          </Text>
        </View>
      </TouchableOpacity>
    </Link>
...
const RatingDots: React.FC<{ ratings: DailyEntry['ratings'] }> = ({ ratings }) => (
  <View style={styles.ratingDots} accessible accessibilityLabel="Ratings">
    ...
    <View
      style={[styles.dot, styles.dotSpacing, { backgroundColor: getRatingColor(ratings.mood) }]}
    />
    <View
      style={[styles.dot, styles.dotSpacing, { backgroundColor: getRatingColor(ratings.energy) }]}
    />
  </View>
);
